...
When you submit a job description, the app makes 6 parallel Gemini API calls — one for each of these sections: summary, experience, skills, education, proof of skill, and certifications. All calls run server-side via API routes at `app/api/tailor/route.ts` and `app/api/cover-letter/route.ts`; your `NEXT_PUBLIC_GEMINI_API_KEY` is never exposed to the browser.
